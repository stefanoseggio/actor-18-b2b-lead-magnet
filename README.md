...
### OSM Overpass discovery order (`osmOverpass` mode)

Overpass has no offset/cursor "page 2" concept for an ad hoc tag query like this one - a single call always returns matching elements in Overpass's own internal storage order (not sorted by relevance, distance, or recency), and this Actor takes the first `maxLeads` of them. **For a bounding box with more qualifying businesses than `maxLeads`, a single run will never reach the elements past that leading subset.** This is an inherent constraint of a stateless per-call Overpass query, not a bug to "just fix" with a bigger page size.

What this Actor *does* do about it: every OSM-sourced `record_id` (`osm:<type>/<id>`) already persisted in the seen-leads store (see above) is excluded from the Overpass query itself, server-side, via Overpass QL's difference operator - regardless of whether `skipKnownLeads` is enabled (exclusion always runs; `skipKnownLeads` only controls whether an already-seen match is then re-enriched or skipped). Practically, this means: **run the same bbox repeatedly (e.g. on a daily/weekly schedule) and each run's Overpass query itself progresses past every element a prior run already returned**, instead of re-matching (and, with `skipKnownLeads` off, re-charging for) that same leading subset forever. This turns "one run, capped at `maxLeads`" into "the full bbox, discovered incrementally across N scheduled runs" - still order-dependent and still with no way to jump straight to "elements 5,000-5,050", but no longer stuck re-fetching the same businesses on every repeat run.
