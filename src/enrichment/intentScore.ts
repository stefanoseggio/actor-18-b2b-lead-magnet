...
export const INTENT_SCORE_DISCLAIMER =
    'Heuristic LLM-derived estimate from public website text only. Not a verified fact; not checked against any funding database, ATS, or independent tech-stack scan.';
...
function buildPrompt(crawledText: string): string {
    return [
...
function parseModelOutput(text: string): { score: number | null; rationale: string | null } {
    try {
        const match = text.match(/\{[\s\S]*\}/);
        if (!match) return { score: null, rationale: 'Could not parse a JSON object from the model response.' };
 ...
 */
export async function computeIntentScore(crawledText: string, apiKey: string | undefined): Promise<IntentScoreResult> {
    if (!apiKey) {
        return {
            intentScore: null,
