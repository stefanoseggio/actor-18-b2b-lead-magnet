...
export function createEmptyState(): SeenLeadsState {
    return { seen: {} };
}

export async function loadState(): Promise<SeenLeadsState> {
    const store = await Actor.openKeyValueStore(SEEN_LEADS_STORE_NAME);
    const state = await store.getValue<SeenLeadsState>('state');
...
export async function saveState(state: SeenLeadsState, newlySeenIds: string[], runAt: string): Promise<SeenLeadsState> {
    const merged = { ...state.seen };
    for (const id of newlySeenIds) merged[id] = runAt;
    ...
    const next: SeenLeadsState = { seen: capped };
    const store = await Actor.openKeyValueStore(SEEN_LEADS_STORE_NAME);
