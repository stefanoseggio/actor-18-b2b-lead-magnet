...
import { crawlWebsite, extractDomain } from './enrichment/websiteCrawl.js';
...
async function enrichCandidate(candidate, input, isNew) {
    const scrapedAt = new Date();
    let crawl = UNAVAILABLE_CRAWL;
    ...
    if (candidate.website) {
        try {
            ...
            log.warning(`Website crawl failed for ${candidate.website}: ${error instanceof Error ? error.message : String(error)}`);
    ...
    let intentScoreRationale = null;
    let intentScoreDisclaimer = null;
...
function buildCrawledText(name, crawl) {
